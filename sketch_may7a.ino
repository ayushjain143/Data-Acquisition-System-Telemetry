...
SoftwareSerial telemetrySerial(10,11); // Create a software serial object for telemetry transmission
  ...
  if (CAN_MSGAVAIL == CAN.checkReceive())
  {
    ...
    if(rxId==2147485360){
      ...
      //Motor Temperature
        ...
        data.mctempmotor = 0.1*hexToDec(mctempmotorhex);

      
      //Controller Temperature
        String mctempcontrollerhex = hex1 + hex2;
        data.mctempcontroller = 0.1*hexToDec(mctempcontrollerhex);
    ...
    //BMS CODE
      ...
      //CURRENT
      String c = hex1 + hex2;
      data.pack_current=(hexToDec(c))/10;
      
      //VOLTAGE
      String v = hex3+hex4;
      data.pack_inst_voltage=(hexToDec(v))/10;
      
      //STATE OF CHARGE      
      data.state_of_charge = buf[4]/2;
      
      //TEMPERATURE
      data.high_temp = buf[5];
      data.low_temp = buf[6];
    }
